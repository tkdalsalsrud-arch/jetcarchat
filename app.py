...
if not st.session_state.form_submitted:
    ...
    with st.form("consultation_form"):
        ...
        with col2:
            ...
            # 🚨 [수정] 용도에 '기타' 추가
            purpose = st.multiselect("차량 사용 용도 (복수 선택 가능)", ["출퇴근용", "영업/업무용", "패밀리카(가족여행)", "레저/캠핑", "장보기/마실용", "기타"])
            
        # 🚨 [신규] 기타 용도 직접 입력란 (폼 내부 col2 아래)
        custom_purpose = st.text_input("기타 용도 직접 입력 (위에서 '기타' 선택 시 작성)", placeholder="예: 낚시용, 대형견 탑승 등")
        ...
        if submit_with_info:
            ...
            # 🚨 [수정] 질문이 없으면 AI에게 자동으로 추천 요청
            if initial_query.strip():
                st.session_state.first_query = initial_query
            else:
                st.session_state.first_query = "위의 [고객 프로필] 정보를 바탕으로 나에게 가장 적합한 차량을 추천해줘. 추천 이유도 설명해줘."
            ...
            # 🚨 [수정] 질문이 없으면 '안내 멘트'만 출력 (AI 호출 X)
            if initial_query.strip():
                st.session_state.first_query = initial_query
            else:
                # 봇의 첫 인사말을 강제로 대화 기록에 추가
